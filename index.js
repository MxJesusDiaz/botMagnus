...
bot.on("message", async message => {
   ...
   if(cmd === `${prefix}play`){
   if (!message.member.voiceChannel) return message.channel.send('Porfavor conectate a un canal de voz.');
   if (message.guild.me.voiceChannel) return message.channel.send('Error! Ya estas conectado a un canal de voz.');
   if (!args[0]) return message.channel.send('Porfavor introduce una url.');

   let validate = await ytdl.validateURL(args[0]);
   if (!validate) return message.channel.send('Introduce una url **valida**.');
